...
<p align="center">
  <a href="README.en.md">English</a> · <a href="README.md">한국어</a> · 日本語 · <a href="README.zh-Hans.md">简体中文</a> · <a href="README.zh-Hant.md">繁體中文</a> · <a href="README.es.md">Español</a> · <a href="README.fr.md">Français</a> · <a href="README.de.md">Deutsch</a>
</p>
...
押している間だけ話す方が好みですか？設定で**プッシュトゥトーク**モードに切り替えると、キー（右`⌘`のような単一の修飾キーでも可）を押している間だけ話し、離すと文字起こしされます。
...
- **2つの録音モード** — *トグル*（押して開始/停止）または*プッシュトゥトーク*（押している間だけ話す、右`⌘`のような単一の修飾キーでも可）の2モードがあり、それぞれ独自のホットキーを設定できます
...
| 操作 | ショートカット |
|---|---|
| 録音開始 / 停止（トグルモード） | `Option + Space`（変更可能） |
| 押している間だけ話す（プッシュトゥトークモード） | `Right ⌘`を押し続ける（変更可能） |
| 録音キャンセル | `Escape` |
| 録音モードの切り替え | 設定 → 一般 → 録音モード |
