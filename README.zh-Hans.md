...
<p align="center">
  <a href="README.en.md">English</a> · <a href="README.md">한국어</a> · <a href="README.ja.md">日本語</a> · 简体中文 · <a href="README.zh-Hant.md">繁體中文</a> · <a href="README.es.md">Español</a> · <a href="README.fr.md">Français</a> · <a href="README.de.md">Deutsch</a>
</p>
...
更喜欢按住说话？可在设置中切换到**按住说话**模式，说话时只需按住某个按键 — 哪怕只是单个修饰键，例如右 `⌘` — 松开后即可转录。
...
- **两种录音模式** — *切换模式*（按一下开始/再按一下停止）或*按住说话*模式（按住即说，哪怕只是单个修饰键，例如右 `⌘`），各自拥有独立的快捷键
- **菜单栏应用** — 安静地驻留在状态栏中
...
vvrite 使用 [`mlx-community/Qwen3-ASR-0.6B-8bit`](https://huggingface.co/mlx-community/Qwen3-ASR-0.6B-8bit)，这是 [`Qwen/Qwen3-ASR-0.6B`](https://huggingface.co/Qwen/Qwen3-ASR-0.6B) 的 MLX 转换版本。根据 Qwen 官方模型卡片，Qwen3-ASR-1.7B 支持 30 种语言和 22 种中文方言的语言识别与语音识别。
...
| 操作 | 快捷键 |
|---|---|
| 开始 / 停止录音（切换模式） | `Option + Space`（可自定义） |
| 按住说话（按住说话模式） | 按住 `Right ⌘`（可自定义） |
| 取消录音 | `Escape` |
| 切换录音模式 | 设置 → 通用 → 录音模式 |
| 打开设置 | 点击菜单栏图标 → 设置 |
...
| ASR 模型 | [Qwen3-ASR-0.6B-8bit](https://huggingface.co/mlx-community/Qwen3-ASR-0.6B-8bit) |
